...
def extractMWValues(benchmark):
    ...
    tHashAll = [[] for op in range(len(OP))]
    tWriteAll = [[] for op in range(len(OP))]
    repetitionResults = []
    for line in f:
        ...
        if flag != 0 and "Handler" in line:
            ...
            if sec > startRep + repetitionSec:
                ...
                if repetition > repetitionNo:
                    return tMWAll, tQueueAll, tServerAll, tHashAll, tWriteAll
            line = next(f)
            ...
            tHashAll[flag].append(metrics[4])
            tWriteAll[flag].append(metrics[5])
            tMWAll[0].append(metrics[1])
            tQueueAll[0].append(metrics[2])
            tServerAll[0].append(metrics[3])
            tHashAll[0].append(metrics[4])
            tWriteAll[0].append(metrics[5])
...
THASH= [[[[] for r in R[0]] for mc in S] for op in range(len(OP))]
TWRITE= [[[[] for r in R[0]] for mc in S] for op in range(len(OP))]
...
for mcIndex in range(len(S)):
    for rIndex in range(len(R[0])):
        for wIndex in range(len(W)):
            ...
            r = R[mcIndex][rIndex]
            print w, mc, r
            benchmark = benchmarkPath + "/queuing_{}_{}_{}_mw.log".format(w, mc, r)
            tMWAll, tQueueAll, tServerAll, tHashAll, tWriteAll = extractMWValues(benchmark)
            for op in range(len(OP)):
                ...
                THASH[op][mcIndex][rIndex].append(mean(tHashAll[op]))
                TWRITE[op][mcIndex][rIndex].append(mean(tWriteAll[op]))
            ...
            for client in range(1, clientNo + 1):
                benchmark = benchmarkPath + "/queuing_{}_{}_{}_{}.log".format(w, mc, r, client)
        ...
        for op in range(len(OP)):
            ...
            THASH[op][mcIndex][rIndex] = [a/1000 for a in THASH[op][mcIndex][rIndex]]
            TWRITE[op][mcIndex][rIndex] = [a/1000 for a in TWRITE[op][mcIndex][rIndex]]
            for pIndex in range(len(percentiles)):
...
TREST = [[[[TMW[op][mcIndex][rIndex][wIndex] - TQUEUE[op][mcIndex][rIndex][wIndex] - TSERVER[op][mcIndex][rIndex][wIndex] for wIndex in range(len(W))] for rIndex in range(len(R[0]))] for mcIndex in range(len(S))] for op in range(len(OP))]
...
print "TWRITE:", TWRITE[0][mcIndex][rIndex][wIndex], TWRITE[1][mcIndex][rIndex][wIndex], TWRITE[2][mcIndex][rIndex][wIndex]
